...
async function validaEscolherCarteira(pergunta) {
  while (true) {
    ...
    if (number > 0 && number < 162) {
      ...
      console.log(
        chalk.bgRed("Erro: voce precisa escolher um numero entre 1 e 161")
      );
    }
  }
}

async function escolherCarteira(pergunta, direto = null) {
  const answer = !direto ? await validaEscolherCarteira(pergunta) : direto;
  ...
  console.log(
    ...
    chalk.yellow(parseInt(BigInt(max) - BigInt(min)).toLocaleString("pt-BR"))
  );
