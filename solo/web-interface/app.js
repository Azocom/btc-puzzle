...
const perguntarParaIniciarInterface = (rl) => {
  return new Promise((resolve, reject) => {
    rl.question(
      "[beta] Deseja iniciar a interface web? (s/n) [n]: ",
      (resposta) => {
              ...
              authMiddleware = basicAuth({
                ...
                unauthorizedResponse: (req) =>
                  req.auth
                    ...
                    : "Necessário fornecer credenciais",
              });
              resolve(true);
            });
          });
...
const buscarSaldos = async (enderecosParaConsulta) => {
  try {
    ...
    const response = await axios.get(
      `https://blockchain.info/balance?active=${enderecosParaConsulta}`
    );
...
const descobrirSaldoNasCarteiras = async () => {
  ...
  if (
    ...
    cache.numChaves === numChaves
  ) {
    return cache.saldo;
  }

  const chavesPublicas = chavesPrivadas.map((chavePrivada) =>
    generatePublic(chavePrivada)
  );
