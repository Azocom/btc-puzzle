...
async function encontrarBitcoinsWorker(key, min, max, workerData) {
  ...
  console.log(
    `Worker para Bloco ${blocoId}: Buscando Bitcoins de forma aleatória...`
  );
  ...
  const executeLoop = async () => {
    while (running) {
      if (workerData.found) {
        console.log(
          `Worker para Bloco ${blocoId}: Busca interrompida - chave encontrada em outro processo.`
        );
        return;
      }

      const randomOffset = BigInt(
        Math.floor(Math.random() * Number(totalChaves))
      );
      ...
      if (Date.now() - startTime > segundos) {
        ...
        const progressoPercentual =
          (BigInt(chavesVerificadas) * BigInt(100)) / totalChaves;

        console.log(
          `Worker para Bloco ${chalk.blue(blocoId)}: ${chalk.yellow(
            segundos / 1000
          )} segundos\nProgresso: ${chalk.red(
            progressoPercentual.toString(),
            "%"
          )} concluído`
        );
        ...
        if (segundos % 10000 === 0) {
          ...
          console.log(
            "Velocidade:",
            chavesVerificadas / tempo,
            " chaves por segundo"
          );
          console.log(
            "Chaves buscadas: ",
            chavesVerificadas.toLocaleString("pt-BR")
          );
      ...
      if (walletsSet.has(publicKey)) {
        const tempo = (Date.now() - startTime) / 1000;
        console.log(
          `Worker para Bloco ${blocoId}: Velocidade:`,
          chavesVerificadas / tempo,
          " chaves por segundo"
        );
        console.log(`Worker para Bloco ${blocoId}: Tempo:`, tempo, " segundos");
        console.log(
          `Worker para Bloco ${blocoId}: Public key:`,
          chalk.green(publicKey)
        );
        console.log(
          `Worker para Bloco ${blocoId}: Private key:`,
          chalk.green(pkey)
        );
        console.log(
          `Worker para Bloco ${blocoId}: WIF:`,
          chalk.green(generateWIF(pkey))
        );

        const filePath = "keys.txt";
        const lineToAppend = `Worker para Bloco ${blocoId}: Public key: ${publicKey},Private key: ${pkey}, WIF: ${generateWIF(
          pkey
        )}\n`;
  ...
  console.log(
    `Worker para Bloco ${blocoId}: Processo interrompido ou concluído.`
  );
}
